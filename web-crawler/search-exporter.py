...
if __name__ == '__main__':
    ...
    output = pd.DataFrame(columns=["id", "question"])

    for q in qs:
        output = output.append({"id": q.id, "question": q.title}, ignore_index=True)

    output.to_csv(os.path.join('../data/', 'results_' + term + '.csv'))
